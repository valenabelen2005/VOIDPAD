  ...
  return (
    <span className="flex gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg
          key={i}
          className={`w-3.5 h-3.5 ${i < count ? 'text-cyan' : 'text-muted/30'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
...
const reviewers = [
  {
    ...
    country: 'Spain',
    date: 'March 2026',
    product: 'Void Series 90×40',
    stars: 5,
    key: 'reviews.1' as const,
  },
  {
    name: 'Julie R.',
    country: 'Netherlands',
    date: 'Feb 2026',
    product: 'Void Classics',
    stars: 5,
    key: 'reviews.2' as const,
  },
  {
    name: 'Nico B.',
    country: 'Germany',
          ...
          <div key={r.name} className="bg-surface border border-border rounded-2xl p-6 space-y-3 flex flex-col">
            ...
            <div className="pt-2 border-t border-border">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-full bg-[#1e1e2e] border border-border flex items-center justify-center text-xs font-bold text-muted">
                  {r.name[0]}
                </div>
                <div>
